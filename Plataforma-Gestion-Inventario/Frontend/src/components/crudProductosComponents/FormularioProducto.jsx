...
import { validarCodigo } from "../../utils/validacionCodigo";
...
const FormularioProducto = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const { valido, mensaje } = validarCodigo(codigo, productos);
    if (!valido) {
      alert("Corrige el campo Código unico" + mensaje);
    }
    if (
      parseFloat(precio) <= 0 ||
      parseInt(stock) <= 0 ||
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="bg-[#E5E5E5] border border-black rounded-lg p-4 mb-6">
            ...
            <div>
              ...
              />
              {mensajeCodigo && (
                <p className="mt-1 text-sm text-red-600">{mensajeCodigo}</p>
              )}
            </div>
